...
### Environment
- `.env` file configured for MySQL (production and development).
- `APP_KEY` generated.
- See [docs/database_initialization_commands.md](docs/database_initialization_commands.md) for one-time MySQL user setup commands.

## Notes for AI Assistant

### Key Reminders
1. **Respect the generic terminology**: Don't rename Groups to Projects (or similar domain-specific renames) without explicit user request.
2. **The algorithm is the core feature**. The Laravel implementation is a port of the Java reference at `/home/nikitas/programming/java/project-group-splitter-java`. When changing algorithm behavior, update [docs/algorithm.md](docs/algorithm.md).
3. **Preference handling**: Today the algorithm reads `rank` as ordering only — there is no unranked-mode toggle. An unranked mode is in the roadmap; don't assume it exists.
4. **Manual adjustment matters**: Teachers rely on quickly moving students between groups after a run. Don't regress that flow.
